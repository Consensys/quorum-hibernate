...
func (c *CliqueConsensus) getCurrentBlockNumber() (int64, error) {
	...
	if err := core.CallRPC(c.client, c.cfg.BasicConfig.QuorumClient.BcClntRpcUrl, []byte(BlockNumberReq), &result); err != nil {
		return 0, err
	}
	if result.Error != nil {
...
func (c *CliqueConsensus) getSigners() ([]string, error) {
	...
	if err := core.CallRPC(c.client, c.cfg.BasicConfig.QuorumClient.BcClntRpcUrl, []byte(CLiqueSigners), &result); err != nil {
		return nil, err
	}
	if result.Error != nil {
