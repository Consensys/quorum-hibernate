...
func (r *IstanbulConsensus) getIstanbulSealerActivity(rpcUrl string) (*IstanbulSealActivity, error) {
	var respResult IstanbulSealActivityResp
	if err := core.CallRPC(rpcUrl, []byte(IstanbulStatusReq), &respResult); err != nil {
		return nil, err
	}
	return &respResult.Result, respResult.Error
}

func (r *IstanbulConsensus) getIstanbulIsValidator(rpcUrl string) (bool, error) {
	var respResult IstanbulIsValidatorResp
	if err := core.CallRPC(rpcUrl, []byte(IstanbulIsValidatorReq), &respResult); err != nil {
		return false, err
	}
	return respResult.Result, respResult.Error
}
