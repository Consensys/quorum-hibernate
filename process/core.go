...
func IsProcessUp(cfg types.UpcheckConfig) (bool, error) {
	if cfg.IsRpcResult() {
		...
		if _, err := core.CallRPC(cfg.UpcheckUrl, cfg.Method, []byte(cfg.Body), &resp, false); err != nil || resp.Error != nil {
			log.Info("IsProcessUp - failed", "err", err, "resp", resp)
			return false, core.ErrNodeDown
		}
	} else if cfg.IsStringResult() {
		if resp, err := core.CallRPC(cfg.UpcheckUrl, cfg.Method, []byte(cfg.Body), nil, true); err != nil {
			log.Info("IsProcessUp - failed", "err", err, "resp", resp)
			return false, core.ErrNodeDown
		} else {
